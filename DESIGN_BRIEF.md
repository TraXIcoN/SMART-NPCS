...
- **Bottom-up personality is the memory stream (§3).** Observations accumulate; the importance-threshold reflection pass distills them into durable beliefs; those beliefs are folded back into the NPC's prompt — so the prompt the model sees literally grows from what the NPC has lived. No retraining; personality is emergent state in encrypted memory. The beliefs sit in a **second, semi-stable cache segment** after the immutable base persona, so growth doesn't thrash prompt caching (it only invalidates on a new reflection, which is rare).
- **What "smart" means here.** With a frozen base model, growth is *knowledge* (more memories), *recall* (the retriever), and above all *abstraction* (reflection turning experience into beliefs) — not raw reasoning. Reasoning capacity moves only by routing salient NPCs to a bigger model, or, longer-term, per-NPC fine-tuning/LoRA on local compute. Abstraction is the growth engine; reflection, not raw memory volume, is what makes an NPC feel wiser.
